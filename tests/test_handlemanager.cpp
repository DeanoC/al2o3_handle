...
TEST_CASE("Basic tests Virtual Dynamic", "[al2o3 handle]") {
	Handle_DynamicManager32* dmanager = Handle_DynamicManager32Create(sizeof(Test), 16);
	REQUIRE(dmanager);
...
TEST_CASE("Block allocation tests Virtual Dynamic", "[al2o3 handle]") {
	...
	Handle_DynamicManager32* dmanager = Handle_DynamicManager32Create(sizeof(Test), AllocationBlockSize);
	REQUIRE(dmanager);
	Handle_Manager32* manager = Handle_Manager32CreateFromDynamic(dmanager);
	REQUIRE(manager);
		...
		*(uint64_t *)Handle_Manager32HandleToPtr(manager, handle) = allocReleaseCycles;
		...
		for (auto i = 0u; i < 100; ++i) {
			...
			if(*test != allocReleaseCycles) {
				LOGINFO("Total handles allocated %u", Handle_Manager32HandleAllocatedCount(manager));
...
TEST_CASE(" Multithreaded Virtual Dynamic", "[al2o3 handle]") {
	...
	Handle_DynamicManager32* dmanager = Handle_DynamicManager32Create(sizeof(uint64_t), blockSize);
	REQUIRE(dmanager);
	Handle_Manager32* manager= Handle_Manager32CreateFromDynamic(dmanager);
